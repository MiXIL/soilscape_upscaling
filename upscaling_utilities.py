...
def colour_SM_image(inimage, colorimage, max_value=0.5, band=1):
    """
    Colour image 
    ...
    if max_value == 0.5:
        ...
        gdalformat = get_gdal_format(colorimage)
        datatype = rsgislib.TYPE_8INT
        color = []
        for i in range(11):
           color.append(colorimage.replace('.kea',str(i)+'.kea'))
           imagecalc.bandMath(color[i], expression[i], gdalformat, datatype, bandDefns)

        bandDefns = []
        for i in range(11):
            SMclass = 'SM'+str(i)
            bandDefns.append(BandDefn(SMclass,color[i],1))
            if i == 0: 
                expression = '('+SMclass
            else:
        ...
        imagecalc.bandMath(colorimage, expression, gdalformat, datatype, bandDefns)       
        for i in range(11):
            os.system('rm '+color[i])

        # Populate stats (converts to RAT):
        rastergis.populateStats(colorimage,False,False,True,ratband=1)

        # Add class field:
        bandStats = []
        bandStats.append(rastergis.BandAttStats(band=1,maxField="SMclass"))
        rastergis.populateRATWithStats(colorimage,colorimage,bandStats,ratband=1)
        ...
        rastergis.colourClasses(colorimage,field,classcolours)

    else:
        raise ValueError('Failed to find colors for specified max_value')
